...
rebase:
	...
	conda run -n test311 python -c 'import cmp3.tests; cmp3.tests.test()';
reset:
	git reset HEAD~1

test: beautiful
	conda run -n base python make/env.py test_cmp3 --python=3.11 --recreate >/dev/null;
	conda run -n test_cmp3 pip install -e . >/dev/null;
	conda run -n test_cmp3 pip install mypy >/dev/null;
	conda run -n test_cmp3 python -m mypy --exclude build --strict .;
	conda run -n test_cmp3 python -m mypy --strict -p cmp3;
